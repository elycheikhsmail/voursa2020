...
from baseApp.forms import ImageLocationsForm
...
def create(request,vente_id, lang):
    ...
    vente = models.CommonLocation.objects.filter(pk=vente_id).first()
    ...
    if request.method == 'POST':
        ...
        print("")
        if len(lst) < 2:
            return render(request,'imagesLocationsApp/create_pb.html' )
        else:
            if lst[-1] not in allow_extensions :
                return render(request,'imagesLocationsApp/create_pb.html' )


        form = ImageLocationsForm(request.POST, request.FILES)  # , instance=img
        #print('form')
        ...
        form = ImageLocationsForm
        context = {'form':form, 'lang':lang}
        return render(request,'imagesLocationsApp/create.html',context)
...
def delete(request,img_id,vente_id, lang):
    ...
    if request.user.is_authenticated :
        if request.method == 'POST':
           ...
           # print(vente)
            img = models.ImageLocations.objects.filter(pk=img_id).first()
            if img is None:
                print('img is Non')
                return HttpResponseRedirect(reverse('commonLocations:index_user_vente',args=(lang,)))
            else:
                ...
                return HttpResponseRedirect(
                    reverse('commonLocations:details_users_vente',args=(vente_id,lang))
                    )
        else:
            context = {'lang':lang}
            return render(request,'imagesLocationsApp/delete.html',context)
    else:
